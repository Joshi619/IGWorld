...
open class ALImageGalleryViewController:UIViewController, UIPageViewControllerDataSource, UIScrollViewDelegate {
    ...
    open func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
        if scrollViewActive {
            ...
            var index:Int = images.firstIndex(of: ((viewController.view.subviews[0] as! UIScrollView).subviews[0] as! UIImageView ).image!)!
    ...
    open func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        mainView.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height - statusBarHeight())
...
//             mainView.frame = CGRect(x: 0, y: statusBarHeight(), width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height - statusBarHeight())
    ...
    func resetPager(){
        ...
        let index:Int = images.firstIndex(of: ((pageViewController.viewControllers!.first!.view?.subviews[0] as! UIScrollView).subviews[0] as! UIImageView ).image!)!
        var temp:[UIViewController] = []
        temp.append(getViewControllerForIndex(index)!)
        pageViewController.setViewControllers(temp, direction: UIPageViewController.NavigationDirection.forward, animated: false, completion:nil)
    ...
    func createMainView(){
        ...
        mainView = UIView(frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height - statusBarHeight()))
//        mainView = UIView(frame: CGRect(x: 0, y: statusBarHeight(), width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height - statusBarHeight()))
    ...
    func createPageController(){
        ...
        pageViewController.setViewControllers(temp, direction: UIPageViewController.NavigationDirection.forward, animated: true, completion:nil)
        ...
        closeButton.setTitle(closeButtonTitle, for: UIControl.State())
...
internal class ALHelperViewController: UIViewController, UIGestureRecognizerDelegate{
    ...
    @objc func handleLongPressGesture(_ sender: UILongPressGestureRecognizer!){
        ...
        case UIGestureRecognizer.State.began:
            let activityController = UIActivityViewController(activityItems: [imageView.image ?? UIImage()], applicationActivities: [])
